...
\begin{def.} \textbf{The Diamond Problem} \\ 
	...
	$$\mu(B_{d}(n)) \in  \bigcup_{k=1}^dP(k)$$
	We need to get a lower bound to show that $\mu(B_{d}(n)) \in  P(d)$\\
	One candidate inequality we could star with is: \\
	$$n^d < \mu(B_{d}(n))$$
	Once we have proven that $\mu(B_{d}(n)) \in P(d)$ we can then know the following equation always has a solution. \\ 
	$$A_d x = b_d$$
	Where: 
	$$A^n_d = 
	\begin{bmatrix}
	...
	\end{bmatrix}
	$$
	And is $\mu(B_{d}(n)) \in P(d)$ then we have $d+1$ unknowns and $d+1$ equations. \\ 
\end{def.}

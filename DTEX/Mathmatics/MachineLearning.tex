...
\begin{def.} \textbf{Equivalence Relations} \\ 
	\begin{itemize}
		\item $a \approxeq a$ \hfill [Reflexive]
		\item $a \approxeq b \Leftrightarrow b \approxeq a$ \hfill [Symmetric]
		\item $a \approxeq b \wedge b \approxeq c \Rightarrow a \approxeq c$ \hfill [Transitive] 
	\end{itemize}
...
\section{Surjective Continuous Non-decreasing Bounded Functionals}
	...
	\begin{thm} \textbf{The K - Famlies form an equivalence class on $\Lambda$} \\
		Let: 
			$$f \approxeq g \Leftrightarrow f,g \in \Lambda(K)$$
			Then $\approxeq$ is an equivalence relation on $\Lambda$ \\ 
	\end{thm}

	\begin{lma} \textbf{Convexity} \\
		Let $f,g \in \Lambda(K)$ for some $K \geq 0$ \\ 
		$$||\lambda f(x) + (1-\lambda)g(x) - (\lambda f(y) + (1-\lambda)g(y)) || = ||\lambda f(x) + (1-\lambda)g(x) - \lambda f(y) - (1-\lambda)g(y) ||$$
		$$ = ||\lambda(f(x) - f(y)) + (1-\lambda)(g(x) - g(y))|| \leq \lambda||(f(x) - f(y))|| + (1-\lambda)||(g(x) - g(y))||$$
		$$\leq \lambda K||x-y|| + (1 - \lambda) K||x - y|| = K||x-y||$$
		Thus: $h(x) = \lambda f(x) + (1-\lambda)g(x)$ is also in $\Lambda(K)$ \\ 
		Since $f$ and $g$ were arbitrary, then $\Lambda(K)$ is convex. \\ 
		Since $K$ was also arbitrary, then every $\Lambda(K)$ is convex. \\  
	\end{lma}

	\begin{lma} \textbf{New operators} \\ 
		Let $f \in \Lambda(K)$ and $g \in \Lambda(L)$ then $h_\lambda(x) := \lambda f(x) + (1-\lambda) g(x)$ belongs to the family: $\Lambda(\lambda K + (1-\lambda) L)$
		Further: $H : [0,1] \to \Gamma $ where $H(\lambda) = h_\lambda$ is a path through the function space $\Gamma = \bigcup_{\nu \in [0,1]} \Lambda(\nu K + (1-\nu) L)$ \\ 

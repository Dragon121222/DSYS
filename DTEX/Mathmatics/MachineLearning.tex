...
\begin{def.} \textbf{The Diamond Problem} \\ 
	...
	To answer this, we need the following equation: 
	...
	$$B_{2}(n) = \bigcup_{j=-n}^n T(B_{1}(n-|j|),j ) = \bigcup_{j=-n}^{-1} T(B_{1}(n-|j|),j ) \cup \bigcup_{j=1}^n T(B_{1}(n-|j|),j ) \cup T(B_{1}(n),0 )$$

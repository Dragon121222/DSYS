...
\begin{def.} \textbf{Logical Deduction} \\
	...
	If we can then show that $Q$ is true, then proven that $P \Rightarrow Q$
\end{def.}
\begin{thm} \textbf{First Theorem} \\ 
	...
	$\vdash A \Rightarrow A \lor A$ \hfill [Rule of Substitution.] \\	
\end{thm}

\begin{thm} \textbf{Second Theorem} \\
	Let $A$ be a statement. \\ 
	Then $A \Leftrightarrow A \lor A $ \\ 	
	Since $A \Rightarrow A \lor A$ and $A \lor A \Rightarrow A$ \\
	Then by definition of equivalence we know: $A \Leftrightarrow A \lor A$ 
\end{thm}

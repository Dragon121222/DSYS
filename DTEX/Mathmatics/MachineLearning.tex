...
\begin{def.} \textbf{Sets and Set Builder Notation} \\ 
	Set builder notation is a way of describing a set using mathematical or logical symbols. \\
...
\section{Surjective Continuous Non-decreasing Bounded Functionals}
	...
	\newpage
	\section{K-Operators}
	Let $\hat{f} : X \to Y$ where $X,Y$ are topological spaces. \\ \\
	Let $V \subset X$ and $W \subset Y$ where $card(V) < card(\mathbb{N})$ and $card(W) < card(\mathbb{N})$
	Then define $f : V \to W$ where the associated topological spaces are: \\ 
	$$\tau_V = \{V \cap U : U \in \tau_X\} \text{ and } \tau_W = \{W \cap U : U \in \tau_Y\}$$
